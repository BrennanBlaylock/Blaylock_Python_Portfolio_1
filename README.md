...
In this analysis we used Python Fundamentals
...
```python
# Types of data
# There are threee common types of data
# Integer numbers
...
```
    ...
    weight in kilograms in now: 65.0
...
```python
if (1 > 0) or (-1 >= 0):
    print('at least one pat is true')
else:
...
```python
def explicit_farh_to_celsius(temp):
    convert = ((temp - 32) * (5/9))
    return converted 
```
...
print('freezing ponit of water in Kelvin:', celsius_to_kelvin(0.))
```

    freezing ponit of water in Kelvin: 273.15
...
```python
def offset_mean(data, target_mean_value):
    """Return a new array continaing the original datra with its mean offset to match the desired value"""
    return(data - numpy.mean(data)) + target_mean_value
```
...
```python
def offset_mean(data, target_mean_value):
    """Return a new array conatining the original data with its mean offset to match the desired value.
...
def offset_mean(data, target_mean_value = 0.0):
    """Return a new array conatining the original data with its mean offset to match the desired value, (0 by default).
...
```
    ...
    <ipython-input-2-415b2d9979e1> in normalize_rectangle(rect)
